// components/Favorito.jsx
"use client";

import { useFavoritos } from "@/context/FavoritosContext"; // ← Importar

export default function Favorito({ peliData }) { // ← Recibir peliData como prop
    
  const { esFavorito, toggleFavorito } = useFavoritos();
  const isFavorito = esFavorito(peliData.id);
    ...
    <div className="bg-black/60 backdrop-blur-sm p-2 rounded-full hover:scale-110 transition-transform duration-200 cursor-pointer">
      ...
      >
        <path
          ...
          className={`hover:fill-red-500/45 transition-colors ease-in-out duration-300 active:fill-red-500 ${
            isFavorito ? "fill-red-500" : "fill-red-500/10"
          }`}
        />
      </svg>
    </div>
